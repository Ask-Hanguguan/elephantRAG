...
def load_rag_prompt():
    try:
        ...
        logger.error(f"[load_rag_prompt]解析系统提示词文件路径失败。")
        raise e

    try:
        return open(rag_prompt_path, "r", encoding="utf-8").read()
    except FileNotFoundError as e:
        logger.error(f"[load_rag_prompt]系统提示词文件{rag_prompt_path}不存在. {str(e)}")
        raise e
    except Exception as e:
        logger.error(f"[load_rag_prompt]解析系统提示词{rag_prompt_path}失败. {str(e)}")
        raise e
